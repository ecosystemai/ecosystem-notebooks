window.dash_clientside = Object.assign({}, window.dash_clientside, {
	clientside: {
		amcharterer: function(clicks, clicks2, data_buffer, data_buffer2) {
			const triggered = dash_clientside.callback_context.triggered.map(t => t.prop_id);
			var used_buffer = [];
			if (triggered == "button.n_clicks"){
				used_buffer = data_buffer;
			}
			if (triggered == "button2.n_clicks"){
				used_buffer = data_buffer2;
			}
		 	am4core.ready(function() {
		 		var target_div = "test_div"
				// Themes begin
				...
				var color_range = 15;
				var cat_list = [];
				new_data = JSON.parse(used_buffer);
				for (i = 0; i < new_data.length; i++) {
					if ("category"  in new_data[i]) {
						cat_list.push(new_data[i]["category"]);
					}
					// if ("colorindex"  in new_data[i]) {
					// 	new_data[i]["color"] = colorSet.getIndex(new_data[i]["colorindex"]);
					// }
				}
